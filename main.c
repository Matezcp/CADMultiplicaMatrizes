...
int main(int argc, char **argv){
    ...
    //Outros processos menos, caso haja mais processo do que o necessário, os processos excedentes
    else if(myRank < N){
        //Aloca a matriz A
        tmp = (int *) malloc (sizeof(int) * N * cargaTrabalho);
        A = (int **) malloc(sizeof(int*) * cargaTrabalho);
    ...
    if(myRank < N){
        //Manda um pedaço da matriz A equivalente a carga de trabalho para cada processo 
        ...
        //#pragma omp parallel for shared(A,B,C,numThreads) private(i,j,k) schedule (static, 10) (NOT WORKING YET)
        for (i = 0; i < cargaTrabalho; i++) {
            for (j = 0; j < N; j++) {
                for (k = 0; k < N; k++) {
                    C[i][j] += A[i][k] * B[k][j];
